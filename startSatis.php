...
if ($_POST['submit'] && $stmt->fetch()) {
	...
	$stmt = $pdo->prepare("UPDATE serv_status SET status = 1, end_time = ? WHERE name = 'satis'");
	$stmt->execute(array($end_time));
	shell_exec('sudo -S /root/startsatis.sh');
	$stmt = $pdo->query("UPDATE serv_status SET status = 2, end_time = NULL WHERE name = 'satis'");
